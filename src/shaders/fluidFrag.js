...
void main() {
  ...
  //vec2 uv = gl_FragCoord.xy / vec2(500.0);
	...
	// using just gl_FragCoord position without normalizing with resolution
  vec2 uv = gl_FragCoord.xy;

  vec4 c = T(uv);

	vec4 n = T(uv + vec2(0,1));
	vec4 e = T(uv + vec2(1,0));
	vec4 s = T(uv - vec2(0,1));
	vec4 w = T(uv - vec2(1,0));
